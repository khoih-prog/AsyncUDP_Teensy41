...
#ifndef ASYNC_UDP_TEENSY41_IMPL_H
#define ASYNC_UDP_TEENSY41_IMPL_H
...
AsyncUDPPacket::AsyncUDPPacket(AsyncUDP *udp, ip_addr_t *localIp, const uint16_t& localPort, ip_addr_t *remoteIp,
...
void AsyncUDP::_recv(udp_pcb *upcb, pbuf *pb, ip_addr_t *addr, const uint16_t& port)
{
...
bool AsyncUDP::listen(ip_addr_t *addr, const uint16_t& port)
{
  close();
  _pcb = udp_new();
...
bool AsyncUDP::listenMulticast(ip_addr_t *addr, const uint16_t& port, uint8_t ttl)
{
  close();
...
bool AsyncUDP::connect(ip_addr_t *addr, const uint16_t& port)
{
  close();
...
size_t AsyncUDP::writeTo(const uint8_t *data, size_t len, ip_addr_t *addr, const uint16_t& port)
{
...
bool AsyncUDP::listen(const IPAddress addr, const uint16_t& port)
{
  ip_addr_t laddr;
  laddr.addr = addr;
...
bool AsyncUDP::listen(const uint16_t& port)
{
  return listen(IPAddress((uint32_t)INADDR_ANY), port);
}

bool AsyncUDP::listenMulticast(const IPAddress addr, const uint16_t& port, uint8_t ttl)
{
  ip_addr_t laddr;
  laddr.addr = addr;
...
bool AsyncUDP::connect(const IPAddress addr, const uint16_t& port)
{
  ip_addr_t daddr;
  daddr.addr = addr;
...
size_t AsyncUDP::writeTo(const uint8_t *data, size_t len, const IPAddress addr, const uint16_t& port)
{
  ip_addr_t daddr;
  daddr.addr = addr;
...
size_t AsyncUDP::broadcastTo(uint8_t *data, size_t len, const uint16_t& port)
{
  ip_addr_t daddr;
...
size_t AsyncUDP::broadcastTo(const char * data, const uint16_t& port)
{
...
size_t AsyncUDP::sendTo(AsyncUDPMessage &message, ip_addr_t *addr, const uint16_t& port)
{
  if (!message)
  {
...
size_t AsyncUDP::sendTo(AsyncUDPMessage &message, const IPAddress addr, const uint16_t& port)
{
  if (!message)
  {
    UDP_LOGDEBUG("sendTo: NULL message. No sendTo");
...
size_t AsyncUDP::broadcastTo(AsyncUDPMessage &message, const uint16_t& port)
{
  if (!message)
  {
...
#endif    // ASYNC_UDP_TEENSY41_IMPL_H
